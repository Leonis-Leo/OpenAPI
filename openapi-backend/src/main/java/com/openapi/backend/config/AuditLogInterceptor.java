...
public class AuditLogInterceptor implements HandlerInterceptor {
 ...
 @Override public void afterCompletion(HttpServletRequest req,HttpServletResponse res,Object handler,Exception ex){try{AuditLog a=new AuditLog();a.setUserId((Long)req.getAttribute("openapi.userId"));a.setAction(req.getMethod());a.setResource(req.getRequestURI());a.setIp(req.getRemoteAddr());a.setStatusCode(res.getStatus());a.setSuccess(res.getStatus()<400?1:0);Map<String,String> p=new HashMap<>();req.getParameterMap().forEach((k,v)->{String l=k.toLowerCase();if(!l.contains("password")&&!l.contains("secret")&&!l.contains("token"))p.put(k,String.join(",",v));});a.setDetail(objectMapper.writeValueAsString(p));mapper.insert(a);}catch(Exception e){log.warn("write audit log failed",e);}}
}
