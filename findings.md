...
- **前端 bug（疑似）**：应用管理「重置密钥」弹窗展示的 SecretKey 疑似被截断——弹窗显示 34 位（`SKbe2763373ab944507a9fce8febdff881(脱敏)`），数据库实际 66 位（`SKbe2763373ab944507a9fce8febdff881(脱敏)...(完整值已脱敏)`）。若"复制"按钮复制的是截断值，会导致开发者拿到无效密钥。**待复核**：打开弹窗核对完整 innerText 与复制内容。
